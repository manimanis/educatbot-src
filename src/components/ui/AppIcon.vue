...
// Bibliothèque d'icônes (style Lucide / Feather)
const ICONS = {
  ...
  exercise: 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z M14 2v6h6 M9 13l2 2 4-4',
  ...
  cap: 'M22 10L12 5 2 10l10 5 10-5z M6 12v5c0 1.66 2.69 3 6 3s6-1.34 6-3v-5',
  ...
  sparkles: 'M12 3l1.9 5.8a2 2 0 0 0 1.3 1.3L21 12l-5.8 1.9a2 2 0 0 0-1.3 1.3L12 21l-1.9-5.8a2 2 0 0 0-1.3-1.3L3 12l5.8-1.9a2 2 0 0 0 1.3-1.3L12 3z M5 3v4 M19 17v4 M3 5h4 M17 19h4',
...
const renderedPath = computed(() => {
  ...
  if (!path) return null
  // Gère plusieurs sous-chemins séparés par ' M' (split intelligent)
  // On les transforme en éléments <path>
  const parts = path.split(/(?= M)/g).filter(Boolean)
  return () => parts.map(p => h('path', { d: p.trim() }))
})
